...
class FrontGearIndexDataType(private val extensionContext: Ki2ExtensionContext) :
    DataTypeImpl(extensionContext.extension, "DATATYPE_FRONT_GEAR_INDEX") {
        ...
        val compositionResult =
            if (connectionInfo?.isConnected == true && shiftingGearingHelper.hasValidGearingInfo()) {
                glance.compose(context, DpSize.Unspecified) {
                    TextView(shiftingGearingHelper.frontGear.toString(),
