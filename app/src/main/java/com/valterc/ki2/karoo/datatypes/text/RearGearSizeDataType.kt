...
class RearGearSizeDataType(private val extensionContext: Ki2ExtensionContext) :
    DataTypeImpl(extensionContext.extension, "DATATYPE_REAR_GEAR_SIZE") {
        ...
        val compositionResult =
            if (connectionInfo?.isConnected == true && shiftingGearingHelper.hasValidGearingInfo()) {
                glance.compose(context, DpSize.Unspecified) {
                    TextView(shiftingGearingHelper.rearGearTeethCount.toString(),
